  ...
  return (
    <Box className="orders">
      ...
      <Typography className="second-title" variant="h4">
        Your Journey to Flavor Begins Here!
      </Typography>
      <Box className="orders-container">
        {isLoading && <Loader />}
        {
          ...
          BookingData.length ? (
            BookingData.map((data, index) => <OrderCard key={index} OrderData={{ data, deleteBookingMutation }} />)
          ) : (
            <Typography
              sx={{ textAlign: 'center', color: 'lightgray', fontSize: '30px', padding: '40px 0' }}
              component="p"
            >
              {' '}
              No Bookings Found
